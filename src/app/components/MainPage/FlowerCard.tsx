  ...
  return (
    <Link href={`/products/${flower.id}`} className="">
      ...
      <div className="flex flex-col justify-between mb-2 px-5">
        <span
          className={`bg-gradient-to-r from-[#006400] via-[#004d00] to-[#003300] shadow ease-in-out hover:from-[#004d00] hover:via-[#003300] hover:to-[#002200] text-white rounded-xl md:rounded-2xl text-xs md:text-sm px-5 py-1 w-fit ${poppins.className}`}
        >
          {language === "pe" ? flower.category_pe : flower.category_en}
        </span>
        <h3
          className={`text-xl lg:text-2xl font-semibold font-parastoo text-start flex-grow mt-3 text-nowrap ${poppins.className}`}
        >
          {language === "pe" ? flower.name_pe : flower.name_en}
        </h3>
        <div className="flex w-full justify-between items-center content-center mt-3">
          <p
            className={`text-black text-opacity-70 text-sm md:text-base ${poppins.className}`}
          >
            {language === "pe" ? flower.type_pe : flower.type_en}
          </p>
          <p
            className={`text-xl text-green font-semibold ${poppins.className}`}
          >
